...
main(int argc, char **argv)
{
    ...
    int i;
    char *multiUserProgs[SIZE];   // for multitasking (running multiple user-level programs)
    int indexProg = 0;            // indicate which program will be loaded
    char *debugArg = "";
    ...
    for (i = 1; i < argc; i++) {
		...
		multiUserProgs[indexProg] = argv[i + 1];
		indexProg++;
        }
    ...
    for (int i = 0; i < indexProg; i++)
    {
        Thread *userprog = new Thread(multiUserProgs[i]);
        userprog->Fork((VoidFunctionPtr)RunUserProg, (void *)multiUserProgs[i]);
    }
